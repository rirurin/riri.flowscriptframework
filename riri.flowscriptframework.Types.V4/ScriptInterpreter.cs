...
public unsafe struct ScriptInterpreter
{
    ...
    [FieldOffset(0x68)] public fixed long StackValue[48]; // should be nint but that can't be used for a fixed buffer
    ...
    [FieldOffset(0x220)] public int StartProc;
    [FieldOffset(0x224)] public int MessageID;
    ...
    [FieldOffset(0x250)] public nint Field250;
    [FieldOffset(0x258)] public ScriptInterpreter* Prev;
    [FieldOffset(0x260)] public ScriptInterpreter* Next;
    [FieldOffset(0x268)] public nint UserData0;
    [FieldOffset(0x270)] public nint UserData1;
    [FieldOffset(0x278)] public nint UserData2;
    [FieldOffset(0x280)] public nint UserData3;
    ...
    public void SetStackType(int Index, StackType Value)
    {
        if (Index > 46) throw new Exception($"Attempted to access out of bounds stack value type {Index}");
        fixed (byte* pStackType = StackType) { ((StackType*)pStackType)[Index] = Value; }
    }
    ...
    public IArgLifetime SetIntValue(int Index, int Value)
    {
        if (Index > 46) throw new Exception($"Attempted to access out of bounds stack value type {Index}");
        fixed (long* pStackValue = StackValue) { *(int*)(pStackValue + Index) = Value; }
        return new LocalFlowInt();
    }
    
    public IArgLifetime SetFloatValue(int Index, float Value)
    {
        if (Index > 46) throw new Exception($"Attempted to access out of bounds stack value type {Index}");
        fixed (long* pStackValue = StackValue) { *(float*)(pStackValue + Index) = Value; }
        return new LocalFlowFloat();
    }
    
    public IArgLifetime SetStringValue(int Index, string Value)
    {
        if (Index > 46) throw new Exception($"Attempted to access out of bounds stack value type {Index}");
        fixed (long* pStackValue = StackValue)
        {
            var pValue = Marshal.StringToHGlobalAnsi(Value);
            *(nint*)(pStackValue + Index) = pValue;
            return new LocalFlowString(pValue);
        }
    }
